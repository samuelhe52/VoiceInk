...
struct Qwen3ASRModelCardView: View {
    let model: Qwen3ASRModel
    @ObservedObject private var modelManager = Qwen3ASRModelManager.shared
        ...
        HStack(alignment: .top, spacing: 16) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 12) {
                    ...
                    Label(model.quantization, systemImage: "number.square")
                    Label("MLX GPU", systemImage: "gpu")
                }
                .font(.system(size: 11))
